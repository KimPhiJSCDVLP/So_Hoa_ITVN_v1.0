...
namespace ITVN.Security
{
    public class TokenProvider : ITokenProvider
    {
        ...
        public bool ValidateAction(TokenIdentity tokenIdentity)
        {
            if (tokenIdentity.UriApi == null || tokenIdentity.UriClient == null)
                return false;
            if (tokenIdentity.UserId == 0)
                return false;
            //var lstAction = GetChucNang.GetChucNangApi(tokenIdentity.UserId);
            //if (lstAction.Count == 0)
            //    return false;
            //if (lstAction.Where(x => x.ChucNang.LinkRouter.ToUpper().Contains(tokenIdentity.UriClient.ToUpper()) 
            //&& x.Api.Uri.ToUpper().Contains(tokenIdentity.UriApi.ToUpper()) && x.Active.Value).Count() != 0)
            //    return true;
            return false;
        }
    }
